...
region_worker() {
  ...
  local idx="$2"
  local total="$3"
  local cache_sku="$CACHE_DIR/skus_$region.json"
  local cache_usage="$CACHE_DIR/usage_$region.json"
...
# --- Logging functions (move up for early use) ---
log() { echo "[INFO] $*"; }
warn() { echo "[WARN] $*" >&2; }
err() { echo "[ERROR] $*" >&2; }
...
# --- Parallel region data collection with caching and progress ---
mkdir -p "$CACHE_DIR"
region_worker() {
  local region="$1"
  local idx="$2"
  ...
  printf "[PROGRESS] [%d/%d] Processing region: %s\n" "$idx" "$total" "$region"
  if [[ $REFRESH_CACHE -eq 1 || ! -f "$cache_sku" ]]; then
    az vm list-skus --location "$region" --resource-type virtualMachines -o json > "$cache_sku" || echo "[ERROR] Failed to fetch skus for $region" >&2
  fi
  if [[ $REFRESH_CACHE -eq 1 || ! -f "$cache_usage" ]]; then
    az vm list-usage --location "$region" -o json > "$cache_usage" || echo "[ERROR] Failed to fetch usage for $region" >&2
  fi
}
